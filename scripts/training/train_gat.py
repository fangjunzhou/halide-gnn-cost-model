"""Train a PipelineModel with a GAT encoder on Halide pipeline graphs."""
...
from halide_gnn_cost_model.model import PipeGAT, PipelineModel


LOGGER = logging.getLogger(__name__)
DEFAULT_PIPELINES_DIR = Path("resources/pipelines-16k-data")
DEFAULT_MODELS_DIR = Path("resources/models/gat")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a GAT-based pipeline runtime regressor."
    )
    parser.add_argument(
        "--pipelines-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Hidden size within the GAT encoder.",
    )
    parser.add_argument(
        "--out-channels",
        type=int,
        default=64,
        help="Output size of the GAT encoder (also the PipelineModel input size).",
    )
    parser.add_argument(
        "--num-layers",
        type=int,
        default=4,
        help="Number of layers in the GAT encoder.",
    )
    parser.add_argument(
        "--gat-heads",
        type=int,
        default=1,
        help="Number of attention heads per GAT layer.",
    )
    parser.add_argument(
        "--gat-dropout",
        type=float,
        default=0.5,
        help="Dropout rate applied inside the GAT encoder.",
    )
    parser.add_argument(
        "--gat-add-self-loops",
        action="store_true",
        help="Add self-loops inside GAT convolutions.",
    )
    parser.add_argument(
        "--model-hidden",
        type=int,
